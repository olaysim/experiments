...
//        ExecutorService threadPool = Executors.newFixedThreadPool(500);
        ...
        dhtConf.setK(200000);
        dhtConf.setA(3);
        dhtConf.setBucketSize(bucketsize);
        ...
        while ((line = bufferedReader.readLine()) != null) {
            ...
            if (sn == null) {
                ...
                KademliaOverlay on = (KademliaOverlay) OverlayFactory.createKademlia(name, netconf, dhtConf, null, sl);
                sn = on.getSuperNode();
                ...
                double y = Double.parseDouble(strings.get(2));
                String name = strings.get(0).trim();
                KademliaOverlay on = (KademliaOverlay) OverlayFactory.createKademlia(name, netconf, dhtConf, sn, sl);
                on.getSelf().putDict("x", strings.get(1));
                on.getSelf().putDict("y", strings.get(2));
                map.put(name, on);
            }
...
//            KademliaOverlay ona = map.get("348");
//            KademliaOverlay targeta = map.get("293");
//            KademliaOverlay ona = map.get("722");
//            KademliaOverlay targeta = map.get("881");
//            KademliaOverlay ona = map.get("303");
//            KademliaOverlay targeta = map.get("216");
//
//            Node nodea = ona.findNode(targeta.getSelf().getId());
//            Node nodeb = ona.findNode(targeta.getSelf().getId());
//
//            int breakpoint = 0;
...
//                        end = rand.nextInt(19999) + 1;
                ...
                Node node = on.findNodeLocal(target.getSelf(), 0 ,0);
                if (node == null) {
                ...
                try {
                    if (routing.size() == 0) System.out.println("WHAAAAT");
...
//                                itrend = rand.nextInt(19999) + 1;
                            ...
                            KademliaOverlay itrTarget = null;
                            if (network == 1 || network == 3 || network == 4) {
                            ...
                            final KademliaOverlay finalItrTarget = itrTarget;
                            threadPool.submit(() -> {
//                            tasks.add(() -> {
                                try {
                                    Node res = entry.getValue().findNodeLocal(finalItrTarget.getSelf(), 0, 0);
                                    if (res == null) {
//                                        log.warn("miss null");
                                        misses.missesSum += 1;
                                    }
...
//        slout.write();
        ...
        Path file_subtotal = Paths.get("data", OUTPUT_FOLDER, "kadem_results_subtotal_2" + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_steps    = Paths.get("data", OUTPUT_FOLDER, "kadem_results_steps_2"    + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_spp      = Paths.get("data", OUTPUT_FOLDER, "kadem_results_spp_2"      + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_factor   = Paths.get("data", OUTPUT_FOLDER, "kadem_results_factor_2"   + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_misses   = Paths.get("data", OUTPUT_FOLDER, "kadem_results_misses_2"   + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_bsize    = Paths.get("data", OUTPUT_FOLDER, "kadem_results_bsize_2"    + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
        Path file_bcsize   = Paths.get("data", OUTPUT_FOLDER, "kadem_results_bcsize_2"   + network + iterativenetwork + "-" + bucketsize + "_" + df.format(Instant.now()) + ".csv");
